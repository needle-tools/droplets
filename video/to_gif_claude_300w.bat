...
if !dur! GTR !maxdur! (
    ...
    %ffmpeg% -i "%~1" -filter_complex "setpts=PTS*!maxdur!/!dur!,fps=14,scale=300:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=256:stats_mode=full[p];[s1][p]paletteuse=dither=floyd_steinberg" -y "%~n1.gif"
) else (
    echo No speedup needed
    %ffmpeg% -i "%~1" -filter_complex "fps=14,scale=300:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=256:stats_mode=full[p];[s1][p]paletteuse=dither=floyd_steinberg" -y "%~n1.gif"
)
